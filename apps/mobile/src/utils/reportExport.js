 ...
 */
export async function fetchAttendanceReportCsv(authToken, { sessionId, classId, corte, mode } = {}) {
  ensureApi();
  const sid = String(sessionId || '').trim();
  const cid = String(classId || '').trim();
  if (!sid && !cid) {
    throw new Error('Falta el identificador de sesión o de clase');
  }
  if (!authToken) {
  ...
  if (corte) body.corte = String(corte).trim();
  if (mode) body.mode = String(mode).trim();
...
export async function exportAttendanceReport(authToken, sessionId, format, options = {}) {
  const { corte, classId, mode } = options;
  const csvText = await fetchAttendanceReportCsv(authToken, { sessionId, classId, corte, mode });
  ...
  const fileKey = String(sessionId || classId || 'informe').replace(/[^a-zA-Z0-9_-]/g, '_');

  if (format === 'csv') {
    const name = `reporte_asistencia_${fileKey}.csv`;
